...
Optimizing Microservice Chain Placement in Kubernetes using Deep Reinforcement Learning
...
2. [Phân Chia Công Việc](#2-phân-chia-công-việc)
3. [Kiến Thức Nền Tảng](#3-kiến-thức-nền-tảng)
4. [Cài Đặt Môi Trường](#4-cài-đặt-môi-trường)
5. [Cấu Trúc Dự Án](#5-cấu-trúc-dự-án)
6. [Bước 1: Xây Dựng Topology Mạng](#6-bước-1-xây-dựng-topology-mạng)
7. [Bước 2: Xây Dựng Kubernetes Environment](#7-bước-2-xây-dựng-kubernetes-environment)
8. [Bước 3: Xây Dựng Agent (DRL)](#8-bước-3-xây-dựng-agent-drl)
9. [Bước 4: Huấn Luyện Model](#9-bước-4-huấn-luyện-model)
10. [Bước 5: Đánh Giá Model](#10-bước-5-đánh-giá-model)
11. [Bước 6: Thực Nghiệm và Kết Quả](#11-bước-6-thực-nghiệm-và-kết-quả)
...
Nhóm gồm **3 thành viên**, phân chia công việc chi tiết như sau:

| Thành viên | Nhiệm vụ chính | Chi tiết công việc |
|------------|----------------|-------------------|
| **Thành viên 1** | Xây dựng môi trường (Environment) | - Implement `envs/topology.py`: Mô hình hóa topology mạng, quản lý node và tài nguyên<br>- Implement `envs/k8s_env.py`: Xây dựng Kubernetes Environment theo chuẩn Gymnasium<br>- Thiết kế State space, Action space, Reward function |
| **Thành viên 2** | Xây dựng Agent (DRL) | - Implement `agents/ppo_agent.py`: PPO Agent<br>- Implement `agents/dqn_agent.py`: DQN Agent<br>- Implement `agents/a2c_agent.py`: A2C Agent<br>- Viết `train.py`: Script huấn luyện model |
| **Thành viên 3** | Đánh giá & Tích hợp | - Viết `evaluate.py`: Script đánh giá model<br>- Viết `main.py`: Entry point tích hợp toàn bộ hệ thống<br>- So sánh với baselines (Random, First-Fit)<br>- Viết báo cáo và tài liệu |

---

## 3. Kiến Thức Nền Tảng

### 3.1 Reinforcement Learning Cơ Bản
...
### 4.4 Bước 3: Cài Đặt Dependencies
...
## 6. Bước 1: Xây Dựng Topology Mạng
...
## 7. Bước 2: Xây Dựng Kubernetes Environment
...
## 8. Bước 3: Xây Dựng Agent (DRL)

### 8.1 Sử Dụng Stable Baselines3
...
## 11. Bước 6: Thực Nghiệm và Kết Quả
